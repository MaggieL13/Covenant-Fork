...
export async function buildOrientationContext(ctx: HookContext, includeStatic = true, userMessage = ''): Promise<string> {
  ...
  if (existsSync(cliPath) && shouldInjectToolReference(ctx, userMessage)) {
    ...
    parts.push([
      ...
      '  fireAt accepts wall-clock in the user\'s identity timezone',
      '  ("2026-04-26 09:00") OR an absolute ISO with Z/offset',
      '  ("2026-04-26T12:00:00Z" / "2026-04-26T09:00:00-03:00").',
      '  Offsetless wall-clock is interpreted in identity.timezone.',
      '',
